...
if (!empty($_POST)) {
	...
	if ($id > 0) {
		...
		if ($thumbnail == '' && $imgs == '') {
			...
			$sql = "update db_product set avatar = '$thumbnail', img = '$imgs', name = '$title', price = $price, detail = '$description', catid = '$category_id', modified_by = '$user_' where id = $id";
		} elseif ($thumbnail != '') {
			...
			$item = executeResult($sql, true);
			deleteFiles($item['img']);
			$sql = "update db_product set img = '$imgs', name = '$title', price = $price, detail = '$description', catid = '$category_id', modified_by = '$user_' where id = $id";
		}
